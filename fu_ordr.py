...
def clear_all_futures_options():
    """선물/옵션 일괄 청산 함수"""
    try:
        # 1. 계좌 정보 및 비밀번호 설정 (실제 계좌 및 비밀번호로 변경)
        account_number = "your_account_number"
        account_password = "your_account_password"

        # 2. CpTrade.CpTd6033 객체 생성 (주문 관련 객체)
        obj_trade = win32com.client.Dispatch("CpTrade.CpTd6033")

        # 3. 계좌 정보 설정
        obj_trade.SetInputValue(0, account_number)
        obj_trade.SetInputValue(1, account_password)

        # 4. 선물/옵션 일괄 청산 주문 실행
        obj_trade.BlockRequest()

        # 5. 주문 결과 확인
        order_result = obj_trade.GetDibStatus()
        order_message = obj_trade.GetDibMsg1()

        if order_result == 0:
            print("선물/옵션 일괄 청산 성공")
        else:
            print(f"선물/옵션 일괄 청산 실패: {order_message}")
...
amount = 1
retOrder = {}
objOrder.buyOrder(code, price,amount, retOrder)
