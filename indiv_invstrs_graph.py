...
class Cp7221:
    def __init__(self):
        ...
        self.data = {key: {'개인매도': [], '개인매수': [], '개인순매수': [], '외국인매도': [], '외국인매수': [], '외국인순매수': [], '기관매도': [], '기관매수': [], '기관순매수': []} for key in self.InvestIndex.values()}
    ...
    def Request(self):
        ...
        for key, value in self.InvestIndex.items():
            ...
            for data_key, data_value in dicInvest.items():
                self.data[value][data_key].append(data_value)
    ...
    def update_graph(self, frame):
        plt.clf()
        for key, value in self.InvestIndex.items():
            plt.plot(self.data[value]['개인순매수'], label=f'{value} 개인순매수')
            plt.plot(self.data[value]['외국인순매수'], label=f'{value} 외국인순매수')
            plt.plot(self.data[value]['기관순매수'], label=f'{value} 기관순매수')
        plt.legend()
        plt.title('투자자별 순매수 동향')
        plt.xlabel('시간')
        plt.ylabel('순매수량')
...
if __name__ == "__main__":
    if InitPlusCheck():
        objMarket = Cp7221()
        fig = plt.figure()
        ani = animation.FuncAnimation(fig, objMarket.update_graph, interval=3000) # 3초마다 업데이트
        plt.show()

        try:
            while True:
                if not objMarket.Request():
                    break
                time.sleep(3)  # 3초 대기
