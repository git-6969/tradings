...
class BalanceThread(LogThread):
    def run(self):
        ...
        if objBalance.request(balanceList):
            if not balanceList:
                self.log_signal.emit("\ud83d\udcdd 현재 보유한 포지션이 없습니다.", False)
                return
            for item in balanceList:
                ...
                self.log_signal.emit(" | ".join(formatted_items), False)
        else:
            self.log_signal.emit("잔고 조회 실패", False)
    ...
    def run(self):
        ...
        if objNContract.request(nContractList):
            if not nContractList:
                self.log_signal.emit("\ud83d\udcdd 현재 미체결 주문이 없습니다.", False)
                return
            for item in nContractList:
                details = " | ".join([f"[{k}] {v}" for k, v in item.items()])
                self.log_signal.emit(details, False)
        else:
            self.log_signal.emit("미체결 조회 실패", False)
            ...
            try:
                while True:
                    ...
                    if objNContract.request(nContractList) and nContractList:
                        self.log_signal.emit(f"\ud83d\udea9 미체결 {len(nContractList)}건 발견 → 취소 시도", False)
                        for order in nContractList:
                            ...
                            if objCancel.cancel_order(order_no, code, qty):
                                self.log_signal.emit(f"\ud83d\udd01 취소 완료: {code} / 주문번호 {order_no}", False)
                            else:
                                self.log_signal.emit(f"⚠️ 취소 실패: {code} / 주문번호 {order_no}", False)
                        time.sleep(1)
                    else:
                        self.log_signal.emit("✅ 모든 미체결 주문 취소 완료", False)
                        break

                balanceList = []
                if objBalance.request(balanceList):
                    if not balanceList:
                        self.log_signal.emit("✅ 모든 포지션 청산 완료!", False)
                        return

                    self.log_signal.emit(f"🔍 현재 잔고 스냅샷 ({len(balanceList)}개 종목):", False)
                    for item in balanceList:
                        self.log_signal.emit(f"    - {item.get('종목명', item.get('코드'))}: 수량 {item['잔고수량']}, 포지션 {item['잔고구분']}", False)
                    ...
                    for item in balanceList:
                        ...
                        if position_type == '매수':
                            ...
                            self.log_signal.emit(f"⚠️ 알 수 없는 포지션 타입: {position_type}", False)
                            continue

                        if success:
                            self.log_signal.emit(f"✅ {code} {qty}개 {action} 주문 성공 (가격: {price})", False)
                        else:
                            self.log_signal.emit(f"❌ {code} {qty}개 {action} 주문 실패", False)

            except Exception as e:
                self.log_signal.emit(f"🚨 예외 발생: {e}", True)
            time.sleep(5)
...
class TradingApp(QWidget):
    ...
    def log(self, message, bold=False):
        if bold:
            self.output.append(f"<b>{message}</b>")
        else:
